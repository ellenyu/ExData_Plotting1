...
# plot 2,1 Voltage vs time
# plot 1,2 sub_metering 1-3 vs time
...
# plot 2
plot(strptime(paste0(euse$Date,";",euse$Time),"%d/%m/%Y;%H:%M:%S"),
     as.numeric(as.character(euse$Voltage)),type="l",xlab="datetime",
     ylab="Voltage")
...
# plot 4
plot(strptime(paste0(euse$Date,";",euse$Time),"%d/%m/%Y;%H:%M:%S"),
     as.numeric(as.character(euse$Global_reactive_power)),type="l",xlab="datetime",
